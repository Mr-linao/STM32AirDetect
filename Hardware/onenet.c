...
#define AUTH_INFO	"xxxxx"	//产品鉴权信息，如果使用EMQX，产品鉴权信息就是密码
	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				{
					json_value = cJSON_GetObjectItem(json,"LED_Switch");		//获取JSON中的LED_Switch字段
					 if (json_value) // 检查 json_value 是否为 NULL
						{
					...
					json_value = cJSON_GetObjectItem(json,"FAN_Switch");		//获取JSON中的FAN_Switch字段
						if (json_value) // 检查 json_value 是否为 NULL
							{
					if(json_value->valueint)
					{
